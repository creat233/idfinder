...
interface ProductWithMCard extends MCardProduct {
  mcard: MCard;
}

interface UnverifiedProductsCarouselProps {
  onImageClick?: (products: MCardProduct[], mcards: MCard[], productIndex: number) => void;
}

interface GroupedProduct {
  mcardId: string;
  products: ProductWithMCard[];
  currentIndex: number;
  isPaused: boolean;
}

export const UnverifiedProductsCarousel = ({ onImageClick }: UnverifiedProductsCarouselProps) => {
  const navigate = useNavigate();
  const [products, setProducts] = useState<ProductWithMCard[]>([]);
  ...
  const handleImageClick = (product: ProductWithMCard, groupIndex: number) => {
    if (longPressTimer.current) {
      clearTimeout(longPressTimer.current);
      longPressTimer.current = null;
      return;
  ...
  const loadProducts = async () => {
    try {
      const { data: productsData, error } = await supabase
        ...
        .eq('mcards.is_verified', false) // Uniquement les cartes NON vérifiées
  ...
  if (loading) {
    return (
      <div className="mb-8 px-4">
        <div className="relative bg-gradient-to-r from-indigo-400/20 to-purple-400/20 rounded-2xl p-6 border border-indigo-400/30">
            ...
            <div className="absolute top-4 left-4 z-20">
              ...
              >
                <div className="relative">
                  <Avatar className="w-10 h-10 border-2 border-white/80 ring-2 ring-gradient-to-r from-indigo-500 to-purple-500">
                    <AvatarImage src={currentProduct.mcard.profile_picture_url || ''} />
                    <AvatarFallback className="text-white bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 font-bold text-sm">
