...
import { useProductTextTranslation } from '@/hooks/useProductTextTranslation';

const AllMCardProducts = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const { products = [], mcardId = '', ownerName = '', ownerUserId = '', currentLanguage: initialLanguage = 'fr' } = location.state || {};
  ...
  const { currentLanguage, setCurrentLanguage, translateProduct, translateText } = useProductTextTranslation(initialLanguage);
  ...
  useEffect(() => {
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter((p: MCardProduct) => {
        ...
        return (
          translatedProduct.name.toLowerCase().includes(query) ||
  ...
  }, [searchQuery, selectedCategory, products, translateProduct]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-primary/5 to-background pb-24 md:pb-8">
      <div className="sticky top-0 z-40 bg-gradient-to-r from-primary via-purple-600 to-pink-600 shadow-lg">
        <div className="container mx-auto px-3 py-3 sm:px-4 sm:py-4">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div className="flex-1 min-w-0">
              <h1 className="text-lg sm:text-xl font-bold text-white truncate">
                {translateText('Produits & Services')}
              </h1>
              {ownerName && (
                <p className="text-white/90 text-xs sm:text-sm truncate">
                  {translateText('De')} {ownerName}
                </p>
              )}
            </div>
          ...
          <div className="relative">
            ...
            <Input
              placeholder={translateText('Rechercher...')}
              value={searchQuery}
      ...
      <div className="container mx-auto px-4 py-6">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-sm font-medium text-muted-foreground">{translateText('Catégories')}</span>
        </div>
        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
          {categories.map((category: string) => (
            ...
            >
              {category === 'all' ? translateText('Tous') : translateText(category)}
            </Badge>
          ))}
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 gap-4 max-w-md">
          <Card className="border-primary/20 bg-gradient-to-br from-primary/5 to-transparent">
            <CardContent className="p-4 text-center">
              ...
              <p className="text-xs text-muted-foreground">{translateText('Total produits')}</p>
            </CardContent>
          </Card>
          <Card className="border-primary/20 bg-gradient-to-br from-purple-500/5 to-transparent">
            <CardContent className="p-4 text-center">
              ...
              <p className="text-xs text-muted-foreground">{translateText('Affichés')}</p>
            </CardContent>
          </Card>
        </div>
      </div>
        ...
        {filteredProducts.length === 0 ? (
          <div className="text-center py-16">
            ...
            <h3 className="text-xl font-semibold mb-2 text-foreground">
              {translateText('Aucun produit trouvé')}
            </h3>
            <p className="text-muted-foreground">
              {searchQuery
                ? translateText("Essayez avec d'autres mots-clés")
                : translateText('Aucun produit dans cette catégorie')}
            </p>
          </div>
        ) : (
              ...
              return (
                ...
                >
                  <CardContent className="p-0">
                    {product.image_url && (
                      <ProductImageModal imageUrl={product.image_url} product={product} currentLanguage={currentLanguage}>
